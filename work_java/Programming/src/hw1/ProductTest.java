	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if(function == 0) break;
				...
				if(choice == 1) {
					tv[index_TV] = new TV(sc.next(), sc.next(), sc.nextInt(), sc.nextInt(), sc.nextInt(),sc.nextInt());
					manager.addProduct(tv[index_TV]);
					index_TV++;
				}
				else if(choice == 2) {
					refrigerator[index_Refrigerator] = new Refrigerator(sc.next(), sc.next(), sc.nextInt(), sc.nextInt(), sc.nextInt(),sc.nextInt());
				...
				String[] temp = manager.search_product_num(input);
				for(int i=0;i<temp.length;i++){
					System.out.println(manager.search_product_num(input));
				}
				// manager.search_product_num(input);
			}
