 ...
 * 먹을 수 있는 물고기가 있으면, 해당 좌표와 거리를 먹이큐(우선순위큐)에 저장.
 * 먹을 수 있는 물고기가 또 나왔는데, 마지막에 저장한 거리보다 더 큰 거리이면 그만 찾자.
 * 먹이큐에서 물고기를 꺼내서, 먹고, 마리수 증가 및 레벨증가 처리.
 * 그리고 그 먹이까지 이동한 거리를 정답에 누적합.
 * 먹이큐에서 물고기가 안 나오면, 종료
 */

public class 아기상어_HONG {

	static int N;					//맵의 크기
	
	static int[][] map;				//맵을 저장할 배열
	
	static boolean[][] visited;		//맵의 크기와 같은 방문 배열
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		while(true) {
			...
			//먹이 카운트가 레벨만큼 차면, 레벨업 하고 카운트 초기화
			if(cnt == lev) {
				lev++;
				cnt = 0;
			}
	...
	//문제 규칙에 맞게,, 맨 왼쪽 위에 있는 애가 나오게끔 하는 자료구조 하나!!!
